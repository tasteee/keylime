...
export const deleteProject = async (id: string) => {
	const supabaseDeleteResult = await supabase.from('all_projects').delete().eq('id', id)
...
export const loadProject = async (id: string): Promise<any> => {
	const supabaseLoadResult = await supabase.from('all_projects').select('*').eq('id', id).single()
