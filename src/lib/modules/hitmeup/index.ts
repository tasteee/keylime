...
const resolveTokensInStyles = (
  styles: HitMeUpCSSPropertiesT,
  tokens: KeyValueObjectT
): HitMeUpCSSPropertiesT => {
...
const mergeStyles = (
  base: HitMeUpCSSPropertiesT,
  override: HitMeUpCSSPropertiesT
): HitMeUpCSSPropertiesT => {
...
const resolvePresetConfig = (
  ...
  const mergedLabelPrefixStyles = mergeStyles(
    parentConfig.labelPrefixStyles || {},
    presetConfig.labelPrefixStyles || {}
  );

  const mergedLabelStyles = mergeStyles(
    parentConfig.labelStyles || {},
    presetConfig.labelStyles || {}
  );

  const mergedMessageStyles = mergeStyles(
    parentConfig.messageStyles || {},
    presetConfig.messageStyles || {}
  );
...
const applyLevelConfigs = (
  ...
  for (const level of levelKeys) {
    ...
    const mergedLabelPrefixStyles = mergeStyles(
      existingConfig.labelPrefixStyles || {},
      levelConfig.labelPrefixStyles || {}
    );

    const mergedLabelStyles = mergeStyles(
      existingConfig.labelStyles || {},
      levelConfig.labelStyles || {}
    );

    const mergedMessageStyles = mergeStyles(
      existingConfig.messageStyles || {},
      levelConfig.messageStyles || {}
    );
...
const createChainedLogger = (
  store: HitMeUpStoreT,
  overrides: Partial<FinalLogOptionsT>
): ChainedLoggerT => {
  ...
  for (const presetName of presetNames) {
    chainedLogger[presetName] = (...logArgs: any[]): void => {
      ...
      const mergedLabelStyles = mergeStyles(
        presetStyles.labelStyles || {},
        overrides.styles?.labelStyles || {}
      );

      const mergedMessageStyles = mergeStyles(
        presetStyles.messageStyles || {},
        overrides.styles?.messageStyles || {}
      );
